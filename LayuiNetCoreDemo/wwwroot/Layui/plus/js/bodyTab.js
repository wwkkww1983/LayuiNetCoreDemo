...
layui.define(["element", "jquery"], function (exports) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].children != undefined && data[i].children.length > 0) {
                ...
                for (var j = 0; j < data[i].children.length; j++) {
                    ...
                    ulHtml += CreateMenu(data[i].children[j]);
                }
                ulHtml += "</dl>";
            } else {
    ...
    function CreateMenu(root) {
        ...
        if (root.children != undefined && root.children.length > 0) {
            ulHtml += '<dd><a>';
            if (root.icon != undefined && root.icon != '') {
                if (root.icon.indexOf("icon-") != -1) {
                    ulHtml += '<i class="seraph ' + root.icon + '" data-icon="' + root.icon + '"></i>';
                } else {
                    ulHtml += '<i class="layui-icon" data-icon="' + root.icon + '">' + root.icon + '</i>';
                }
            }
            ulHtml += '<cite>' + root.title + '</cite>';
            ulHtml += '<span class="layui-nav-more"></span>';
            ulHtml += '</a>';
            ulHtml += '<dl class="layui-nav-child">';

            for (var k = 0; k < root.children.length; k++) {
                ulHtml += CreateMenu(root.children[k]);
            }
            ulHtml += "</dl>";
            ulHtml += '</dd>';
        }
        else {
            if (root.target == "_blank") {
                ulHtml += '<dd><a data-url="' + root.href + '" target="' + root.target + '">';
            } else {
                ulHtml += '<dd><a data-url="' + root.href + '">';
            }
            if (root.icon != undefined && root.icon != '') {
                if (root.icon.indexOf("icon-") != -1) {
                    ulHtml += '<i class="seraph ' + root.icon + '" data-icon="' + root.icon + '"></i>';
            ...
            ulHtml += '<cite>' + root.title + '</cite></a></dd>';
        }
        return ulHtml;
    }
    ...
    Tab.prototype.render = function () {
        //显示左侧菜单
        var _this = this;
        $(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="/Activity/ActivityList"><i class="layui-icon" data-icon=""></i><cite>后台首页</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height() - 210);
    ...
    Tab.prototype.tabCustomAdd = function (target,titleText,url) {
        if (window.sessionStorage.getItem("menu")) {
            menu = JSON.parse(window.sessionStorage.getItem("menu"));
        }
        var that = this;
        ...
        if (target == "_blank") {
            ...
            var title = '';
                    ...
                    title += '<i class="seraph"></i>';
         
                    //title += '<i class="layui-icon">' + _this.find("i.layui-icon").attr("data-icon") + '</i>';
   
            //已打开的窗口中不存在
            //if (that.hasTab(titleText) == -1) {
                if ($(".layui-tab-title.top_tab li").length == openTabNum) {
                    layer.msg('只能同时打开' + openTabNum + '个选项卡哦。不然系统会卡的！');
                    return;
                }
                tabIdIndex++;
                title += '<cite>' + titleText + '</cite>';
                title += '<i class="layui-icon layui-unselect layui-tab-close" data-id="' + tabIdIndex + '">&#x1006;</i>';
                element.tabAdd(tabFilter, {
                    title: title,
                ...
                window.sessionStorage.setItem("menu", JSON.stringify(menu)); //打开的窗口
                window.sessionStorage.setItem("curmenu", JSON.stringify(curmenu));  //当前的窗口
                element.tabChange(tabFilter, that.getLayId(titleText));
                that.tabMove(); //顶部窗口是否可滚动
            //} else {
            //    //当前窗口内容
            //    var curmenu = {
